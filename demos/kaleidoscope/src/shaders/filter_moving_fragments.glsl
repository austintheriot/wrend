...
void main() {
  ...
  const float DIVIDING_SECTOR = FULL_CIRCLE / 12.0;
  // range from [0.5 to 1.5]
  float rebounding_now = sin(u_now) * 0.5 + 1.0;
  float moving_sector = DIVIDING_SECTOR * rebounding_now;

  // sample only the first 1/12 of the polar coordinate plane
  polar_coords = mod(polar_coords, vec2(10000, moving_sector));
