use super::{texture_id::TextureId, texture_id_number::TextureIdNumber};
...
pub fn crate_prev_frame_a_texture<UserCtx>(ctx: &TextureCreateContext<UserCtx>) -> WebGlTexture {
    let gl = ctx.gl();
    let webgl_texture = gl
        .create_texture()
        .expect("Should be able to create textures from WebGL context");
...
pub fn crate_prev_frame_b_texture<UserCtx>(ctx: &TextureCreateContext<UserCtx>) -> WebGlTexture {
    let gl = ctx.gl();
    let webgl_texture = gl
        .create_texture()
        .expect("Should be able to create textures from WebGL context");
    ...
    gl.active_texture(WebGl2RenderingContext::TEXTURE0 + TextureId::PrevFrameB.num());
    gl.bind_texture(WebGl2RenderingContext::TEXTURE_2D, Some(&webgl_texture));
